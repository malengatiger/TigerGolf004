...
import com.boha.golfpractice.player.util.VideoFileException;
...
public class YouTubeService extends IntentService {
    ...
    private void getVideoList() {
        SnappyVideo.getVideo((MonApp) getApplication(), new SnappyVideo.ReadListener() {
            ...
            public void onDataRead(ResponseDTO response) {
                Log.d(LOG, "... back from Snappy, list: " + response.getVideoUploadList().size());
                if (!response.getVideoUploadList().isEmpty()) {
                    index = 0;
                    ...
                    try {
                        ...
                        youtube = new YouTube.Builder(new NetHttpTransport(),
                                ...
                                .setApplicationName("com.boha.TGolf.App").build();
    ...
    private void uploadToYouTube(final VideoUploadDTO videoUpload, File file) throws VideoFileException {
        ...
        MonLog.d(getApplicationContext(), LOG, "++++++++++++++++ uploadToYouTube\nstarting video upload: "
                + videoUpload.getDateTaken() + "\nfile path: " + videoUpload.getFilePath()
                + "\nlocalUri: " + videoUpload.getFilePath());
        ...
        try {
            ...
            snippet.setTitle(videoUpload.getPlayerName() + ": "
                    + sdf.format(new Date(videoUpload.getDateTaken())));
            snippet.setDescription(
                    "Project video clip from TGolf");
                ...
                public void progressChanged(MediaHttpUploader uploader) throws IOException {
                    switch (uploader.getUploadState()) {
                        ...
                        case INITIATION_COMPLETE:
                            ...
                            mBuilder.setContentTitle(videoUpload.getPlayerName())
                                    .setContentText("YouTube Video Upload starting .... ")
                                    .setSmallIcon(R.drawable.golfball48);
        ...
        private void sendVideoMetadata(final VideoUploadDTO videoUpload) {
            ...
            try {
                ...
                if (resp.getStatusCode() == 0) {
                    MonLog.w(getApplicationContext(), LOG, "Response from server video metadata request, about to update video in cache");
                    List<VideoUploadDTO> list = new ArrayList<>();
                    resp.getVideoUploadList().get(0).setDateUploaded(new Date().getTime());
                    list.add(resp.getVideoUploadList().get(0));

                    SnappyVideo.addVideo((MonApp) getApplication(), list, new SnappyVideo.WriteListener() {
                        @Override
